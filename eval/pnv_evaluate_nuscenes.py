...
def evaluate(model, device, params: TrainingParams, log: bool = False, show_progress: bool = False, degree = 0, rotate=False):
    ...
    eval_database_files = ['Hioformer_nuscenes_evaluation_database.pickle']

    eval_query_files = ['Hioformer_nuscenes_evaluation_query.pickle']

    # eval_database_files = ['Hioformer_kitti_evaluation_database.pickle']

    # eval_query_files = ['Hioformer_kitti_evaluation_query.pickle']
...
def get_latent_vectors(model, set, device, params: TrainingParams, degree=0, rotate=False):
    ...
    for i, elem_ndx in enumerate(set):
        ...
        # pc_file_path = pc_file_path.replace('depth_map', 'PNV')
        # pc_file_path = pc_file_path.replace('png', 'pcd.bin')
...
def compute_embedding(model, pc, device, params: TrainingParams):
    ...
    with torch.no_grad():
        ...
        # print("描述子时间为:", execution_time_ms, "毫秒")
...
def get_recall(m, n, database_vectors, query_vectors, query_sets, database_sets, log=False):
    ...
    for i in range(len(queries_output)):
        ...
        end_time = time.time()
        execution_time_ms = (end_time - start_time) * 1000
        # print("查询时间为:", execution_time_ms, "毫秒")
    print("one_percent_retrieved_{}".format(one_percent_retrieved))
    print("num_evaluated_{}".format(num_evaluated))
    print("recall_{}".format(recall))
...
if __name__ == "__main__":
    ...
    if args.rotate == True:
        for degree in range(30,361,30):
            stats = evaluate(model, device, params, args.log, show_progress=True, degree = degree, rotate=args.rotate)
