    ...
    def __init__(
        ...
        if proj_type == "mlp":
            ...
            self.embed = nn.Sequential(
                nn.Conv2d(
                    in_chans, embed_dim // 4, kernel_size=[5, 1], stride=[1, 1]
                ),  # 60
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    embed_dim // 4, embed_dim // 2, kernel_size=[3, 1], stride=[2, 1]
                ),  # 29
                nn.ReLU(inplace=True),
                nn.Conv2d(
                    embed_dim // 2, embed_dim, kernel_size=[3, 1], stride=[2, 1]
                ),  # 14
                nn.ReLU(inplace=True),
...
class featureExtracter(nn.Module):
    def __init__(
        ...
        window_size=[[1, 14], [1, 28], [1, 56], [1, 112]],
        mlp_ratio=4.0,
        qkv_bias=True,
        qk_scale=None,
        drop_rate=0.0,
        ...
        **kwargs,
        ...
        self.hidden_weights = nn.Parameter(
            torch.randn(1024, 256) * 1 / math.sqrt((embed_dim * 2))
        )
        if self.gating:
    ...
    def forward(self, x_l):
        ...
        if self.use_fpn:
            ...
            pfm = torch.matmul(
                pfm, self.hidden_weights
            )  # B x 8*C X 8*C x 2*C -> B x 2*C
            if self.gating:
