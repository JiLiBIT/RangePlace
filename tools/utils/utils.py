...
def range_projection_HDL64E(
    current_vertex, fov_up=3, fov_down=-25, proj_H=32, proj_W=900, max_range=1
):  # HDL64
    """Project a pointcloud into a spherical projection, range image.
    Args:
      current_vertex: raw point clouds
...
def pad_to_multiples_of_depth(tensor, n1, depth1, n2, depth2, value=0):
    ...
    if pad_h == 0 and pad_w == 0:
        ...
        return (
            2 * pad_h,
            2 * pad_w,
            F.pad(tensor, (pad_w, pad_w, pad_h, pad_h), mode="constant", value=value),
        )
