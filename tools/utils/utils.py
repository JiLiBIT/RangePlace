...
def range_projection_HDL32E(current_vertex, fov_up=10.67, fov_down=-30.67, proj_H=32, proj_W=896, max_range=80, cut_z=False,
                     low=0.1, high=6):
...
def range_projection_HDL64E(current_vertex, fov_up=3, fov_down=-25, proj_H=32, proj_W=900, max_range=1): # HDL64
...
def pad_to_multiples_of(tensor, n, value=0):
    ...
    if pad_h == 0 and pad_w == 0:
        ...
        return F.interpolate(tensor.permute(0, 3, 1, 2), size=(h+pad_h, w+pad_w), mode='bilinear', align_corners=False).permute(0, 2, 3, 1)
        # return F.pad(tensor, (0, pad_h, pad_w, 0), mode = 'constant', value=value)

    
def pad_to_multiples_of_depth(tensor, n1, depth1, n2, depth2, value=0):
    """
    Pad tensor to be a multiple of n .
    ...
    n1_h = int(math.pow(n1[0], depth1))
    n1_w = int(math.pow(n1[1], depth1))
    n2_h = int(math.pow(n2[0], depth2))
    n2_w = int(math.pow(n2[1], depth2))
    n_h = lcm(n1_h,n2_h)
    n_w = lcm(n1_w,n2_w)
    _, _, h, w = tensor.size()
    pad_h = int((n_h - h % n_h) % n_h/2)
    
    pad_w = int((n_w - w % n_w) % n_w/2)
    if pad_h == 0 and pad_w == 0:
        return tensor
    else:
        return 2*pad_h, 2*pad_w, F.pad(tensor, (pad_w, pad_w, pad_h, pad_h), mode = 'constant', value=value)
def add_to_multiples_of_depth(h, w , n1, depth1, n2, depth2, value=0):
    """
    Pad tensor to be a multiple of n .
