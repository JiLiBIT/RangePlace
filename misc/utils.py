...
class ModelParams:
    def __init__(self, model_params_path):
        ...
        assert self.coordinates in ['polar', 'cartesian','rangeimage'], f'Unsupported coordinates: {self.coordinates}'
        ...
        elif 'rangeimage' in self.coordinates:
        ...
        if 'window_size' in params:
            ...
            self.window_size = tuple([e for e in params['window_size'].split(';')])
            self.window_size = [tuple(map(int, s.split(','))) for s in self.window_size]
...
class TrainingParams:
    ...
    def __init__(self, params_path: str, model_params_path: str, debug: bool = False):
        ...
        if self.scheduler is not None:
            ...
            elif self.scheduler == 'WarmupCosine':
                self.min_lr = params.getfloat('min_lr')
                self.warmup_epochs = params.getfloat('warmup_epochs')
                self.warmup_lr_init = params.getfloat('warmup_lr_init')
            else:
                raise NotImplementedError('Unsupported LR scheduler: {}'.format(self.scheduler))
        self.warmup_epochs = params.getint('warmup_epochs', 5)
        self.warmup_lr_init = params.getfloat('warmup_lr_init', 5e-6)
