...
class TrainingDataset(Dataset):
    def __init__(self, dataset_path, query_filename, transform=None, set_transform=None):
        ...
        self.ri_loader: RangeImageLoader = None
    ...
    def __getitem__(self, ndx):
        ...
        query_pc = self.ri_loader(file_pathname)
...
class RangeImageLoader:
    ...
    def __call__(self, file_pathname):
        ...
        # plane and below
        # file_pathname: relative file path
        file_pathname = os.path.splitext(file_pathname)[0]
        file_pathname = ".."+file_pathname + '.png'
        assert os.path.exists(file_pathname), f"Cannot open range image: {file_pathname}"
        ri = self.read_ri(file_pathname)
        assert ri.shape[0] == 1
