...
def construct_query_dict(df_centroids, base_path, filename, ind_nn_r, ind_r_r=50):
    ...
    for anchor_ndx in range(len(ind_nn)):
        ...
        assert (
            os.path.splitext(scan_filename)[1] == ".bin"
        ), f"Expected .bin file: {scan_filename}"
...
if __name__ == "__main__":
    ...
    construct_query_dict(
        df_train, base_path, "training_queries_baseline2.pickle", ind_nn_r=10
    )
    construct_query_dict(
        df_test, base_path, "test_queries_baseline2.pickle", ind_nn_r=10
    )
